...
contract MMR is MMRTree {
    ...
    mapping(uint256 => bytes32) nodeIndexToRoot; // Mapping of node index to relative root hash
    mapping(uint256 => bytes32[]) nodeIndexToPeaks; // Mapping of node index to peaks. Peaks can be calculated and/or stored off-chain

    bytes32[] lastPeaks; // Peaks of the last tree. For illustration purpose, can be calculated and/or stored off-chain
    uint256 lastElementsCount; // Latest elements count
    bytes32 lastRoot; // Latest root hash

    // Emitted event after each successful `append` operation
    event Appended(bytes32 element, bytes32 rootHash, uint256 elementsCount);
    ...
    function append(bytes32 element) external {
        ...
        (uint256 nextElementsCount, bytes32 nextRootHash, bytes32[] memory nextPeaks) =
            StatelessMmr.appendWithPeaksRetrieval(element, lastPeaks, lastElementsCount, lastRoot);
    ...
    function multiAppend(bytes32[] memory elements) external {
        ...
        for (uint256 i = 0; i < elements.length; ++i) {
            (nextElementsCount, nextRoot, nextPeaks) =
                StatelessMmr.appendWithPeaksRetrieval(elements[i], nextPeaks, nextElementsCount, nextRoot);
